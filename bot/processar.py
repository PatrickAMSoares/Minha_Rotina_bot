...
def aplicar(atualizacao):
    """Aplica uma atualizacao do Telegram. Usado aqui e pelo escutar.py."""
    try:
        ...
        print("erro ao tratar atualizacao %s: %s" % (atualizacao.get("update_id"), erro))


def main():
    estado = dados.ler_estado()
    offset = estado.get("offset", 0)
    print("lendo a partir do offset %s" % offset)

    resposta = telegram.buscar_atualizacoes(offset)
    if not resposta.get("ok"):
        # Falha de verdade: o passo precisa ficar vermelho, nao verde.
        print("ERRO ao consultar o Telegram: %s" % resposta.get("erro"))
        return 1

    atualizacoes = resposta.get("result", [])
    if not atualizacoes:
        print("nada novo (nenhum toque ou mensagem esperando)")
        return 0

    for atualizacao in atualizacoes:
        estado["offset"] = atualizacao["update_id"] + 1
        if "callback_query" in atualizacao:
            print("  toque em botao: %s"
                  % atualizacao["callback_query"].get("data"))
        elif "message" in atualizacao:
            print("  mensagem: %r"
                  % (atualizacao["message"].get("text") or "")[:40])
        aplicar(atualizacao)

    dados.gravar_estado(estado)
    print("processadas %d atualizacoes; novo offset %s"
          % (len(atualizacoes), estado["offset"]))
    return 0
